...
Begin VB.Form frmopengrnclosure 
   BackColor       =   &H00EDDDD1&
   Caption         =   " *Open Goods Receipt Closure *"
   ClientHeight    =   9210
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   12465
   Icon            =   "frmopengrnclosure.frx":0000
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   ScaleHeight     =   11115
   ScaleWidth      =   15240
   ...
   Begin VB.PictureBox Checked 
      ...
      Height          =   285
      Left            =   840
      Picture         =   "frmopengrnclosure.frx":0442
      ScaleHeight     =   255
      ScaleWidth      =   645
      TabIndex        =   3
      Top             =   120
   ...
   Begin VB.PictureBox Unchecked 
      ...
      Height          =   255
      Left            =   120
      Picture         =   "frmopengrnclosure.frx":0D4A
      ScaleHeight     =   225
      ScaleWidth      =   705
      TabIndex        =   2
      Top             =   120
   ...
   End
End
Attribute VB_Name = "frmopengrnclosure"
...
Private Sub cmdupdate_Click()
    ...
    rs.Open "Select * from opengrnstatus_details ", cn, 1, 3
    If rs.RecordCount <> 0 Then
    For i = 1 To rs.RecordCount
    If rs.BOF = False Then rs.MoveFirst
    While rs.EOF = False
        rs![opengrnstatus] = GrnMainGrid.TextMatrix(i, 6)
        rs.Update
        rs.MoveNext
        i = i + 1
        Wend
...
Sub gridload()
    Dim i As Integer
    Dim rs As New ADODB.Recordset
    rs.Open "Select * from opengrnstatus_details", cn, adOpenKeyset, adLockOptimistic
    i = 1
    If rs.BOF = False Then rs.MoveFirst
    While rs.EOF = False
    ...
    GrnMainGrid.TextMatrix(i, 6) = rs![opengrnstatus]
...
Private Sub Form_Load()
    ...
    ww.Open "Select * From opengrnstatus_details", cn, adOpenKeyset, adLockOptimistic
    cn.CursorLocation = adUseClient
    
    Call opengrnclosuregridloads
    Call gridload
