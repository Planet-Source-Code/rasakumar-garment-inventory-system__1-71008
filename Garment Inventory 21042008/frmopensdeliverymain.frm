...
Begin VB.Form frmopensdeliverymain 
   BackColor       =   &H00EDDDD1&
   Caption         =   " * Open Delivery Challans *"
   ClientHeight    =   9375
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   12105
   Icon            =   "frmopensdeliverymain.frx":0000
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   ScaleHeight     =   11115
   ScaleWidth      =   15240
   ...
   Begin VB.ComboBox cboDeliveryFilter 
      ...
      TabIndex        =   3
      ToolTipText     =   " To Use Filter The Dc No "
      Top             =   1080
      Width           =   1935
   End
   ...
   Begin VB.CommandButton cmdlist1 
      ...
      Style           =   1  'Graphical
      TabIndex        =   0
      ToolTipText     =   " Show All Delivery 's "
      Top             =   360
      Width           =   1575
   End
   ...
   End
End
Attribute VB_Name = "frmopensdeliverymain"
...
Dim rs As New ADODB.Recordset
    On Error GoTo X
        ...
        Set rs = cn.Execute("select opdcno from deliveryopenmain_details order by opdcno")
        rs.MoveFirst
        Do While Not rs.EOF()
...
Dim rs As New ADODB.Recordset
    On Error GoTo X
        cbodept.Clear
        Set rs = cn.Execute("select deptname from deliveryopenmain_details group by deptname")
        rs.MoveFirst
        Do While Not rs.EOF()
...
Dim rs As New ADODB.Recordset
    On Error GoTo X
        cbosupfilter.Clear
        Set rs = cn.Execute("select supname from deliveryopenmain_details group by supname")
        rs.MoveFirst
        Do While Not rs.EOF()
...
Private Function DeliveryMainGridload()
    ...
    rs.Open "select * from deliveryopenmain_details ", cn, adOpenKeyset, adLockOptimistic
    i = 1
    If rs.BOF = False Then rs.MoveFirst
    While rs.EOF = False
        DeliveryMainGrid.Rows = rs.RecordCount + 1
        DeliveryMainGrid.TextMatrix(i, 0) = i
        DeliveryMainGrid.TextMatrix(i, 1) = rs![opdcno]
        DeliveryMainGrid.TextMatrix(i, 2) = rs![opdcdate]
...
Private Sub cmddelete_Click()
    ...
    Else
        rs.Open "Select * from opendelivery_details where opdcno= '" & frmopensdeliverymain.txtedit.Text & "'", cn, adOpenKeyset, adLockOptimistic
        If rs.RecordCount <> 0 Then
        If MsgBox("Are You Sure Delete Open DC No  " & txtedit.Text & " ? ", vbQuestion + vbYesNo, "Confirm To Delete") = vbYes Then
        i = 1
        If rs.BOF = False Then rs.MoveFirst
        While rs.EOF = False
        rs.Delete
...
Private Sub cmdedit_Click()
    ...
    frmopendelivery.Show
    frmopendelivery.cmdsave.Visible = False
    frmopendelivery.cmdaddrow.Enabled = False
    frmopendelivery.cmddeleterow.Enabled = False
    ...
    If rs.BOF = False Then rs.MoveFirst
    While rs.EOF = False
        frmopendelivery.DeliveryGrid.Rows = rs.RecordCount + 1
        frmopendelivery.txtdcno.Text = rs![opdcno]
        frmopendelivery.dt1.Value = rs![opdcdate]
        frmopendelivery.cbosupplier.Text = rs![supname]
        frmopendelivery.cbodept.Text = rs![deptname]
        frmopendelivery.DeliveryGrid.TextMatrix(i, 0) = rs![sno]
        frmopendelivery.DeliveryGrid.TextMatrix(i, 1) = rs![itemname]
        frmopendelivery.DeliveryGrid.TextMatrix(i, 2) = rs![colour]
        frmopendelivery.DeliveryGrid.TextMatrix(i, 3) = rs![Size]
        frmopendelivery.DeliveryGrid.TextMatrix(i, 4) = Format(rs![deliveryqty], "0.000")
        frmopendelivery.DeliveryGrid.TextMatrix(i, 5) = rs![uom]
        frmopendelivery.txtremarks.Text = rs![remarks]
        rs.MoveNext
        i = i + 1
        Wend
  End If
...
Private Sub cmdprint_Click()
    ...
    rsgrn.Open "SELECT * FROM opendelivery_details where opdcno ='" & txtedit.Text & "'", cn, 1, 3
    ...
    If Trim(txtedit.Text) = "" Then
    MsgBox "Please Select The Open DC No ", vbCritical, "GRN No Selection Error"
    txtedit.SetFocus
    Else
    ...
    Kill App.path & "\Reports\Printdeliveryopened.txt"
    Open App.path & "\Reports\Printdeliveryopened.txt" For Output As #1
   
    Print #1,
    Print #1, Tab(42); "Delivery Challan ( Open )";
    Print #1,
    Print #1, String(100, "-");
    Print #1, Tab(2); "DC No: "; rsgrn![opdcno]; Tab(75); "DC Date :"; rsgrn![opdcdate];
    Print #1,
    Print #1, String(100, "-");
    ...
    Print #1,
    Print #1, String(100, "-");
    Print #1, Tab(2); "S.No"; Tab(8); "Item Name"; Tab(25); "Colour"; Tab(40); "Size"; Tab(59); "Rec.Qty", Tab(71); "UOM";
    
    Print #1,
    Print #1, String(100, "-");
    rsItem.Open "Select * from opendelivery_details where opdcno ='" & txtedit.Text & "'", cn, 1, 3
    i = 1
    If rsItem.BOF = False Then rsItem.MoveFirst
    While rsItem.EOF = False
    Print #1,
    Print #1, rsItem![sno]; Tab(8); rsItem![itemname]; Tab(25); rsItem![colour]; Tab(40); rsItem![Size]; Tab(59); Format(rsItem![deliveryqty], "0.000"); Tab(71); rsItem![uom];
    rsItem.MoveNext
    i = i + 1
    Wend
    Print #1,
...
Private Sub Form_Load()
    ...
    cn.Open "Provider=Microsoft.jet.oledb.4.0; Data Source=" & App.path & "\Database\Data.mdb"
    ww.Open "Select * From deliverymain_details", cn, adOpenKeyset, adLockOptimistic
    Call DeliveryMainGridload
    Call frmopendeliverygridmain
...
Private Function filtersupname()
    Dim rs As New ADODB.Recordset
    rs.Open "select * from deliveryopenmain_details where supname= '" & Trim(cbosupfilter.Text) & "'", cn, adOpenKeyset, adLockOptimistic
    i = 1
    If rs.BOF = False Then rs.MoveFirst
    While rs.EOF = False
...
Private Function filterdcno()
    Dim rs As New ADODB.Recordset
    rs.Open "select * from deliveryopenmain_details where opdcno= '" & Trim(cboDeliveryFilter.Text) & "'", cn, adOpenKeyset, adLockOptimistic
    i = 1
    If rs.BOF = False Then rs.MoveFirst
    While rs.EOF = False
    DeliveryMainGrid.Rows = DeliveryMainGrid.Rows + 1
...
Private Function filterdept()
    Dim rs As New ADODB.Recordset
    rs.Open "select * from deliveryopenmain_details where deptname= '" & Trim(cbodept.Text) & "'", cn, adOpenKeyset, adLockOptimistic
    i = 1
    If rs.BOF = False Then rs.MoveFirst
    While rs.EOF = False
    DeliveryMainGrid.Rows = DeliveryMainGrid.Rows + 1
...
Private Function filtertype()
    Dim rs As New ADODB.Recordset
    rs.Open "select * from deliveryopenmain_details where deliverytype= '" & Trim(cbotypefilter.Text) & "'", cn, adOpenKeyset, adLockOptimistic
    i = 1
    If rs.BOF = False Then rs.MoveFirst
    While rs.EOF = False
    DeliveryMainGrid.Rows = DeliveryMainGrid.Rows + 1
...
Private Function filteralldc()
    Dim rs As New ADODB.Recordset
    rs.Open "select * from deliveryopenmain_details", cn, 1, 3
    i = 1
    If rs.BOF = False Then rs.MoveFirst
    While rs.EOF = False
    DeliveryMainGrid.Rows = DeliveryMainGrid.Rows + 1
